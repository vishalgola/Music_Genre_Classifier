...
def model_prediction(X_test):
    ...
    # 1. Get indices that would sort the 'counts' in ascending order
    # 2. Reverse it [::-1] to get descending order
    # 3. Take the first 3 indices
    top_3_indices = np.argsort(counts)[::-1][:3]

    # 4. Use those indices to get the actual category labels
    top_3_elements = unique_elements[top_3_indices]

    return top_3_elements.tolist()  # Returns a list of the 3 most frequent categories
...
#Prediction Page
    ...
    if test_mp3 is not None:
        # create temporary file instead of saving in Test_Music
        with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp:
            tmp.write(test_mp3.read())
            filepath = tmp.name
